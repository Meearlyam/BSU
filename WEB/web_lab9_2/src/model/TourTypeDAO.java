...
public final class TourTypeDAO extends DAO {
    ...
    public List<TourType> readAllTourTypes() throws TourTypeDAOExecutionException {
        ...
        try {
            ...
            CriteriaQuery<TourType> query = builder.createQuery(TourType.class);
            Root<TourType> root = query.from(TourType.class);
            query.select(root);
            TypedQuery<TourType> typedQuery = manager.createQuery(query);
            tourTypes = typedQuery.getResultList();
        } catch (Exception e) {
